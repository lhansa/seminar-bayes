...
**`index.qmd` está entero**: el seminario se rehizo de cero (issue #36) y recorre la moneda (#39), el caso de Meridian (#40), el esqueleto (#41), el modelo de medios (#43) y el cierre (#44). `_quarto.yml` apunta a él y no renderiza nada más.
...
Las diferencias con la carta son decisiones tomadas, no olvidos, y se mantienen: la anécdota de Obama con la que abre la carta se ha quitado (se empieza con la moneda), la moneda se resuelve solo por simulación (fuera la aproximación normal y el binomial exacto) y la posteriori exacta (una Beta) también se queda fuera: la nota del ponente de `graf-caminata-posteriori` cuenta por qué se puede muestrear sin la constante normalizadora, que se va al comparar dos valores de `p`.

**La correspondencia con el p-valor es exacta con el *mid-p-value*, no con el p-valor de siempre.** Con priori uniforme, `P(p < 0,5 | datos) = P(X > 60) + ½·P(X = 60) = 0,0230`, frente al 0,0284 de la slide. Vive en la nota del ponente de `graf-hdi-moneda`; no escribas que «el número es el mismo».
...
**Otras prioris para la moneda tampoco tienen slide** (#102): qué le pasa a `P(p > 0,5)` con una Beta centrada en 0,6 o una absurda en 0,2 vive solo en la nota del ponente de `prob-sesgo`, con los números medidos en el issue. La priori de la charla sigue siendo la uniforme de la carta.
...
La referencia principal de la charla es **McElreath, *Statistical Rethinking*** (#71): se nombra de palabra en la nota del muestreo —el capítulo del rey Markov— y por eso es la slide de «Dónde profundizar», donde se recoge, no se presenta. Esa sección es hoy solo McElreath: Gelman y Hill y la slide de «Lo que no ha cabido» ya no están en el deck.

La fuente de verdad metodológica del modelo es el repositorio **`google/meridian`** (Apache 2.0): escalado del KPI y de los medios, adstock geométrico normalizado, la priori sobre el ROI y la saturación de Hill, que se ha medido y se deja fuera (más abajo). Si hay que ampliar algo, se mira su código (`meridian/model/`), no la memoria.
...
- Las prioris por defecto de Meridian están en `meridian/model/prior_distribution.py`. Se usan tal cual **salvo dos**: `sigma` pasa de `HalfNormal(5)` a `Exponential(1)` y `gamma` de `Normal(0, 5)` a `Normal(0, 1)`. En un modelo nacional con 156 semanas y el KPI estandarizado, las de Meridian generan ingresos semanales negativos; eso se mide con la simulación previa, no se cuenta. Es una razón de principio, no de resultado: ajustado el esqueleto con las prioris de Meridian, la posteriori sale igual (`gamma` −0,417 frente a −0,415), porque con 156 semanas y tres parámetros los datos las tapan. Lo que se proyecta es una **comparación de predictivas previas**, no un análisis de sensibilidad: no lo llames así.
- **`mu` cambia entre los dos modelos y no es un descuido**: `Normal(0, 1)` en el esqueleto, donde es el nivel de un KPI estandarizado, y `Normal(0, 5)` en el de medios, donde tiene que compensar lo que aportan (su posteriori se va a −2,8; con la otra priori no llegaría). Ese cambio *es* la lección del bloque 3, así que no lo unifiques ni lo "arregles" comparando con `prior_distribution.py`. Y lo que paga la priori apretada es el ROI, no el muestreador, y está medido: con `mu ~ Normal(0, 1)` el muestreo sale limpio, sin divergencias, `mu` se queda en −2,26 y la mediana del ROI de `Channel0` baja de 1,29 a 0,97. Eso es lo que cuenta la nota del ponente, y es el ejemplo de la concesión en «¿no es subjetivo?».
...
- **La posteriori de `alpha` sale casi uniforme por ruido, no por la normalización.** Está medido: con los mismos gastos y un ruido cuatro veces menor, el mismo modelo recupera los `alpha` con los que se simulan los datos. La normalización saca a `alpha` de la contribución total, no del reparto en el tiempo, y ese reparto se distingue mal con 156 semanas ruidosas y `Channel2`, `Channel3` y `Channel4` moviéndose juntos. No escribas que es estructural.
- **El ROI es ingreso incremental entre inversión**, lo que en marketing se llama ROAS. ROI = 1 es recuperar en ventas lo invertido, no el punto muerto de beneficio: con un margen del 30 %, ese está en 3,3. En slides y notas no se escribe «ni gana ni pierde» ni «perder dinero». Y es una afirmación causal que exige que el presupuesto no se decida mirando la demanda esperada; la nota del ponente de «`beta` deja de ser un parámetro» lo dice.
- **El KPI del modelo es la facturación (`revenue`), no las conversiones, y está medido** (issue #89, cerrado como *wontfix*). En `national_all_channels.csv`, `revenue_per_conversion` vale 0,0200 €/conversión con una desviación típica de 0,000015: un coeficiente de variación del **0,076 %**, y las dos series estandarizadas no se separan más de **0,032 σ** en 156 semanas. Con el KPI estandarizado, modelar conversiones es el mismo modelo con otro eje: no cambia una posteriori. Y la cuenta de `beta` sí se ensucia, porque el valor por conversión entra dentro del sumatorio (`beta_m = roi_m · gasto_m / (sd(y) · Σ_t adstock_tm · rpc_t)`), que es un término que en estos datos no hace nada. Encima las conversiones del CSV son **422 millones a la semana**, que no se las cree nadie en la sala, mientras que 8,45 M€ semanales sí, y de eso vive la slide de la simulación previa. **No lo cambies a conversiones.** Que se puede modelar otro KPI se dice de palabra, en la nota del ponente de la slide de `beta`: el modelo es el mismo, pero para escribir la priori sobre el ROI hace falta un euro por unidad, y ese número se da como dato —por eso Google lo trae como columna—.
- **El modelo de medios va sin saturación de Hill, y está medido, no supuesto** (issue #42; se reproduce con `scripts/comparar_hill.py`). Ajustadas las dos versiones sobre los mismos datos, la posteriori de `ec` devuelve su priori: mediana de 0,99 a entre 1,06 y 1,27 según el canal, y la desviación típica clavada en 0,62. Los datos no saben decir dónde satura cada canal. Y como no lo saben, el parámetro de más solo añade incertidumbre: el HDI al 90 % del ROI se ensancha un 60 %, `sigma` sube de 0,839 a 0,859 —el modelo explica menos— y el orden de los canales por ROI no se mueve. Encima cuesta `target_accept` de 0,95 a 0,97 y el doble de muestreo (1,2 min contra 2,5). La saturación se menciona **de palabra**, como lo que no cabe en la charla, en la nota del ponente de la slide del adstock (#43). Desde #73 esos números están además en una slide de *backup* (`{#saturacion}`), escritos a mano en una tabla: **el modelo con Hill no se ajusta en ningún sitio del render**. **No lo metas por tu cuenta.**
- **Lo que sí se mueve con Hill, y por eso la frase final no se cuelga de ello.** Con saturación, tres canales pasan de mediana de ROI por debajo de 1 a por encima (`Channel1` 0,90 → 1,30, `Channel3` 0,96 → 1,40, `Channel4` 0,82 → 1,09) y su probabilidad de no recuperar en ventas lo invertido cae unos veinte puntos. Es un número que se mueve **sin que los datos hayan dicho nada**, porque quien lo mueve es `ec`, que no está identificado. Así que la frase final de #43 se construye sobre `Channel2`, cuyo ROI aguanta las dos versiones, y nunca sobre «este canal no recupera lo que cuesta», que no aguanta ninguna. La comparación con `Channel0` que la acompañaba ya no está. Y `Channel2` es el canal donde más pesa la priori —el de menos inversión, con su HDI al 90 % tan ancho como el de la priori—: la nota del ponente de `frase-roi` lo dice antes de la slide de priori contra posteriori.
- **Sin Hill, el ROI medio y el marginal son el mismo número**, y eso es lo que sostiene la frase de cierre. El ROI del modelo es `contribución_m / gasto_m`, un promedio sobre el gasto observado; «subir presupuesto» es una decisión sobre el euro siguiente, que es el marginal. Coinciden porque el modelo es lineal en el adstock. Es la segunda razón para dejar Hill fuera, y vive en la nota del ponente de `frase-roi` (#61), no en una slide.
- **El modelo de medios muestrea con `target_accept=0.95`**, no con `0.9`: con `0.9` deja 3 divergencias (medido en #42). Los `0.9` del esqueleto se quedan como están, que ahí muestrea limpio.
- **El OLS vive en el esqueleto, no en la apertura del bloque de medios** (issue #87). Ahí es comparable: se ajusta sobre `y_esc`/`x_esc`, las mismas dos variables estandarizadas del modelo `esqueleto`, así que sus tres números caen donde `mu`, `gamma` y `sigma`. Hoy el esqueleto abre con la ecuación de esa regresión, no con la tabla del OLS, y el OLS no se ajusta en el render: sus números —centros −0,42 y 0,10, p-valor del sentimiento 0,21— viven escritos en las notas del ponente. El OLS de cinco canales que abría el bloque de medios se quitó porque no era comparable con ningún modelo de la charla —sin adstock, sin la parametrización en ROI— y necesitaba dos notas del ponente solo para desactivar la comparación que la propia slide invitaba a hacer. No lo devuelvas ahí.
- **El OLS y la posteriori del esqueleto no se comparan en una tabla** (issue #98). La slide que los ponía lado a lado («Mismos datos, mismos números») se sustituyó por «Cada parámetro es una distribución, y se lee en euros», y como tres frases con seis números resultaban muy densas, desde #108 son **dos slides en columnas**: a la izquierda el histograma de la posteriori de una `gamma` en euros semanales y a la derecha su lectura en telegráfico. La primera es la competencia, lejos del cero, con el HDI al 90 % sombreado; la segunda, «Con el sentimiento de marca tenemos muchas dudas», con el área a la derecha del cero en morado, que es el `P(p > 0,5)` de la moneda otra vez. **Los euros van por cada desviación típica** del control, en el texto y en el eje: `gamma · sd(y)` no es «la competencia me quita tanto a la semana». `mu` no se enseña, y `sigma` —la varianza explicada y la desviación típica de lo que una semana se aparta de lo previsto— vive en la nota del ponente de la primera, dicha de palabra porque el bloque de medios la cobra. No se añade una tercera slide por parámetro. El p-valor del sentimiento (0,21) vuelve en la nota del ponente frente al 89 % de la posteriori, **dicho con cuidado**: mismos datos, otra pregunta, no «bayes ve efectos que el p-valor no ve». Y cuadran: a una cola es 0,11, igual que 1 − 0,89. Que los centros coinciden se dice de palabra en esa nota; no devuelvas la tabla comparativa. La misma nota avisa de que en el modelo de medios el sentimiento cambia de signo (P(suma) de un 2 %) por sesgo de variable omitida: la slide enseña a leer una posteriori, no concluye nada del sentimiento.
...
El recorte de #72 **está hecho**. El recorrido son 43 slides de contenido (42 hasta que #108 partió en dos la lectura del esqueleto) y **unos 45 minutos estimados a ojo de slide**, no cronometrados: la moneda ~18, el caso ~3, el esqueleto ~6, los medios ~14 y el cierre ~3. El objetivo eran 40; 45 vale, y lo que no valía eran los 59 de los que se venía. **El número bueno sale del ensayo con cronómetro, no de esa tabla**: no afines al minuto ni presupuestes colchón por interrupciones, que la sala no interrumpe la parte expositiva —para eso están los veinte minutos de después—.

Si el ensayo se pasa, lo primero es `A qué me dedico` de un minuto a treinta segundos. El segundo recorte previsto, mandar la tabla del OLS a *backup*, ya no existe: el esqueleto abre con la ecuación. Y el bloque de la moneda queda en 19 slides, no en las 18 que presupuestaba #72, porque #67 se decidió no hacer.

- **`# Backup` va al final, detrás de la slide de preguntas (`## leonardohansa.com/regalo-bayes {#preguntas}`).** Como toda sección con slides debajo, revealjs la monta en pila vertical: no aparece en el recorrido salvo que se siga pulsando la flecha después del final.
- **Se salta a una slide concreta con `G`, su id y Enter.** Por eso cada una lleva id fijo: `{#priori-plana}`, `{#saturacion}`, `{#ajuste}`, `{#series}`, `{#el-lunes}`. Está probado sobre el deck renderizado, no supuesto. Con `slide-number: false` el salto por número no sirve, así que **si añades una slide de backup, ponle id**. El mismo cuadro busca también por texto, y el menú (☰) solo llega hasta «Backup», no hasta cada slide de la pila.
- **El criterio de qué vive ahí**: la pregunta previsible que se responde mejor con un gráfico que de palabra. Hoy son cinco: la priori plana en logit (#68, que salió del bloque de la moneda), la comparación con y sin Hill, el ajuste contra la serie real (`ajuste`, #112), «El lunes por la mañana» —que es la respuesta a «¿por dónde empiezo?» y es de tres frases accionables, no de bibliografía— y las series temporales del caso (`series`), que salieron del recorrido en #70 y se enseñan solo si alguien pregunta por la pinta de los datos.
- **El ajuste contra la serie real vive en *backup*, no en el recorrido** (#112): la serie de ingresos en morado sobre cien muestras de la predictiva posterior en gris suave. Sale de `idata_medios.posterior_predictive`, que ya se muestrea para el `plot_ppc`, así que no añade MCMC. Lleva el ruido dentro, no la curva media, y es ajuste dentro de muestra: no hay *holdout*, porque la charla explica el ROI, no predice la semana siguiente. En el recorrido se queda el `plot_ppc`, cuya nota dice que la forma encaja —los cuantiles simulados caen sobre los observados— y que lo flojo es cuánto explica. No hay «suelo» que contar: el cero está a catorce desviaciones típicas de la media.
...
- **El guion de las preguntas está en la nota del ponente de la slide de preguntas (`{#preguntas}`)**: las once previstas, en orden de probabilidad, cada una con dónde cae —nota, slide de la charla o *backup*—. Si mueves o añades una slide de *backup*, esa nota se actualiza con ella.
- **La última slide lleva el QR, igual que «Notebook de regalo», y es a propósito**: durante veinte minutos de preguntas hay una slide proyectada delante de la sala, y es el mejor hueco de captación de la charla.
...
- **Coma decimal en el texto que escribe el código**: títulos, leyendas e inline llevan `.replace(".", ",")`. Los números de los ejes de matplotlib se quedan con punto.
...
- Es **narrativo y sin ejercicios**. No hay celdas `# Tu código aquí`, no hay notebook de soluciones y no hay pareja que mantener sincronizada: lo que en el taller archivado es un ejercicio, aquí es texto que responde a la pregunta. No copies la estructura del taller. La única invitación al lector es una frase en markdown (probar `sigma=10` en la priori del Modelo 1), no una celda.
- **El recorrido es el de #110**: un `alpha` común (OLS y bayesiano) → un `alpha` por condado, con los extremos etiquetados, que son condados de dos o tres casas → jerárquico, con el mismo gráfico mostrando el *shrinkage* → diagnóstico y predicción, cortos. Cada EDA justifica una pieza del modelo: plantas → `beta1`, medias por condado → que un `alpha` común no sirve. **Nada de EDA porque sí.**
- **Simulación previa en los Modelos 1 y 2, no en el 3**, y está medido: la previa del jerárquico sale como la del Modelo 2 (percentil 90 de 33 pCi/L contra 22), con la cola algo más larga. Lo dice una frase.
- **El texto es telegráfico**: oraciones cortas, sujeto-verbo-complemento, pocas subordinadas y nada de «no es X, es Y». Si añades texto, que siga así.
