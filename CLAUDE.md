...
**El muestreo se cuenta con un Metropolis a mano, en numpy, y es a propósito** (issue #29). NUTS da muestras casi independientes: su recorrido parece ruido blanco y no se ve ni un paso corto ni un rechazo, que es justo lo que ahí se cuenta. Una nota del ponente avisa de que PyMC usa NUTS y no esto.

Desde #67 el Metropolis vive en **dos slides que ya no están juntas**: en el recorrido se queda `graf-caminata-posteriori`, que enseña los dos mil pasos y el montón que dejan —ahí lo que importa no es el paso, sino el montón—, y el zoom de los `zoom = 200` primeros, donde se ven el paso y el rechazo, se proyecta desde *backup* (`{#caminata}`). Por eso **la cadena y `dibuja_recorrido()` se calculan en la celda de `graf-caminata-posteriori`**, que es la que va antes: la celda de *backup* solo dibuja. Lo que se deja de ver está en la nota del ponente de la slide superviviente, el paso, el rechazo y la subida inicial que explica el `tune`, junto con el aviso de que la slide de *backup* existe.
...
- **El criterio de qué vive ahí**: la pregunta previsible que se responde mejor con un gráfico que de palabra. Hoy son cuatro: la priori plana en logit (#68, que salió del bloque de la moneda), el zoom del Metropolis (#67), la comparación con y sin Hill y «El lunes por la mañana», que es la respuesta a «¿por dónde empiezo?» y es de tres frases accionables, no de bibliografía.
- **El zoom del Metropolis (`graf-caminata`) vive en *backup* desde #67**, con id `{#caminata}`. La decisión anterior era la contraria y cambió al conocerse el formato real: con 20 minutos de preguntas, «¿cómo funciona el muestreo por dentro?» se responde mejor sacando la slide que gastando minutos de charla en ella. La nota del turno de preguntas manda ahí la pregunta 7.
- **El guion de las preguntas está en la nota del ponente de `# leonardohansa.com`**: las ocho previstas, en orden de probabilidad, cada una con dónde cae —nota, slide de la charla o *backup*—. Si mueves o añades una slide de *backup*, esa nota se actualiza con ella.
