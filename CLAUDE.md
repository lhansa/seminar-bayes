...
Seminario de **una hora** sobre inferencia bayesiana en la empresa: slides revealjs en `index.qmd`, sobre los datos de *marketing mix* de Google Meridian. Recorrido:

1. **La moneda.** 60 caras en 100 tiradas, el p-valor por simulación y la vuelta al problema.
2. **El esqueleto.** Nivel y controles del KPI, tres prioris, muestrea en tres segundos.
3. **El modelo de medios.** Adstock geométrico normalizado, saturación de Hill y priori sobre `beta`.
4. **La discusión.** Por qué Meridian pone la priori en el ROI y no en `beta`. Se cuenta, no se implementa.
...
- `index_radon.qmd`: las slides del seminario anterior, de dos horas, con la moneda y el radón de Minnesota (agrupado → *unpooled* → jerárquico). De aquí sale la parte de la moneda.
- `notebooks/taller.ipynb` y `notebooks/taller-solucion.ipynb`: el taller de Meridian que ya no se imparte. De aquí sale el código de los puntos 2 y 3. Habrá un taller nuevo sobre el radón, en otro issue.
- `data/radon.csv` y `notebooks/idata/modelo_meridian.nc`.
...
La fuente de verdad metodológica del modelo es el repositorio **`google/meridian`** (Apache 2.0): escalado del KPI y de los medios, adstock geométrico normalizado, saturación de Hill y la priori sobre el ROI. Si hay que ampliar algo, se mira su código (`meridian/model/`), no la memoria.
...
- **`mu` cambia entre los dos modelos y no es un descuido**: `Normal(0, 1)` en el esqueleto, donde es el nivel de un KPI estandarizado, y `Normal(0, 5)` en el de medios, donde tiene que compensar lo que aportan (su posteriori se va a −2,8; con la otra priori no llegaría). Ese cambio *es* la lección del punto 3, así que no lo unifiques ni lo "arregles" comparando con `prior_distribution.py`.
- La priori sobre `beta` de la charla está por fijar; la justifica la simulación previa y la discusión del punto 4 es su contrapunto. **El taller archivado deduce `beta` de una priori sobre el ROI y no lleva Hill**: es otra decisión, de otro formato, y no hay que unificarlos.
- La saturación de Hill cuesta un parámetro más por canal y se pelea con el adstock por explicar la misma forma: con ella el muestreo pedía `target_accept=0.97`; sin ella bastaba `0.9`. Si el modelo de la charla no muestrea limpio, ese es el primer sitio donde mirar.
...
- **El morado `#800080` es el color de marca** y está en cuatro sitios: `$morado` en `custom.scss`, `ACENTO` en `src/estilo.py` y `ACENTO` en la `celda-01` de cada notebook. Si cambia la marca, se cambia ahí y en ningún sitio más. Las copias de fuera del `.scss` son el precio de que el taller sea autosuficiente en Colab, y es un precio aceptado.
...
- El muestreo de las slides usa `draws=1000, tune=1000` para que el render no se eternice. No lo subas sin necesidad.
