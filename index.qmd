...
resumen_ols_esqueleto = pd.DataFrame({
    "ols": list(ols_esqueleto.params) + [ols_esqueleto.resid.std(ddof=3)],
    "p-valor": list(ols_esqueleto.pvalues) + [np.nan],
}, index=["intercept"] + CONTROLES + ["residuos"])
resumen_ols_esqueleto.round({"ols": 2, "p-valor": 3})
```
...
gamma_sentimiento = idata_esqueleto.posterior["gamma"].sel(control="sentiment_score_control")
p_sentimiento_sube = float((gamma_sentimiento > 0).mean())
p_valor_sentimiento = resumen_ols_esqueleto.loc["sentiment_score_control", "p-valor"]
...
- Si la competencia vende una desviación típica más, facturo entre `{python} euros(-hdi_competencia[1])` y `{python} euros(-hdi_competencia[0])` € menos a la semana.
- Con el sentimiento hay un `{python} f"{100 * p_sentimiento_sube:.0f}"` % de probabilidad de que empuje hacia arriba, pero su efecto va de `{python} euros(hdi_sentimiento[0])` a `{python} euros(hdi_sentimiento[1])` €.
- Los controles explican el `{python} f"{100 * varianza_explicada_esqueleto:.0f}"` % de la varianza: una semana cualquiera se aparta de lo previsto entre `{python} euros(hdi_sigma_euros[0])` y `{python} euros(hdi_sigma_euros[1])` €.

::: aside
Intervalos al 90 % de la posteriori. El efecto de cada control, por cada desviación típica
que se mueve.
:::

::: {.notes}
Con prioris flojas y `{python} len(df)` filas, la priori no manda: los centros son los del
OLS con el que empezó el bloque, y si alguien lo comprueba, coinciden. Quien quiera ver a la
priori mandar que espere al bloque de medios, donde `mu` pasa de `Normal(0, 1)` a
`Normal(0, 5)` porque el modelo es otro. Lo que cambia aquí no es el número, es la lectura:
cada frase lleva su incertidumbre en euros, y se puede decir en una reunión sin traducir.

El sentimiento es el que merece la pausa. En la tabla del OLS salía con un p-valor de
`{python} f"{p_valor_sentimiento:.2f}".replace(".", ",")`: "no significativo", y ahí se
acaba la conversación. Aquí sale un `{python} f"{100 * p_sentimiento_sube:.0f}"` %.
Cuidado con cómo se dice: no es que la bayesiana vea un efecto que el p-valor no ve. Son
los mismos datos respondiendo a otra pregunta. El p-valor dice cómo de raros serían estos
datos si el efecto fuera cero; la posteriori, qué me creo del efecto después de verlos. Es
la vuelta al problema de la moneda, otra vez. Y el `{python} f"{100 * p_sentimiento_sube:.0f}"` %
no autoriza a decir que el sentimiento funciona: el intervalo cruza el cero, y eso también
va en la frase.
