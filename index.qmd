...
- Y los canales se mueven juntos: `Channel2`, `Channel3` y `Channel4` correlacionan entre sí por encima de `{python} f"{min(correlaciones.loc['Channel2', 'Channel3'], correlaciones.loc['Channel2', 'Channel4'], correlaciones.loc['Channel3', 'Channel4']):.2f}"`, y son `{python} len(df)` filas para separarlos.

::: aside
Cinco canales, `{python} len(df)` semanas y la pregunta de la reunión: ¿cuál subo?
:::
...
## La pregunta no es si un canal funciona: es cuánto devuelve

- La respuesta es un número con incertidumbre, no un sí o un no.
- Y ya sabemos, por la moneda, que hay otra manera.

# El esqueleto: nivel y controles del KPI

## Esto lo sabes ajustar de memoria
...
resumen_ols_esqueleto = pd.Series(
    ...
    index=["intercepto"] + CONTROLES + ["error residual"],
    name="ols",
)
