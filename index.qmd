...
<!-- TODO: slide de presentación del ponente. La rellena Leonardo.
     Un minuto y sin currículum, pero con la parte de empresa delante: de dónde vengo,
     qué decisiones he visto tomar con estos modelos y por qué doy este seminario. -->
...
::: aside
Ese número es el p-valor: lo mismo que da el contraste de siempre, pero sin fórmulas.
:::
...
- Tres líneas: lo que creías antes de mirar, las 60 caras de 100, y la binomial que las genera.
...
- Y los canales se mueven juntos: `Channel2`, `Channel3` y `Channel4` correlacionan entre sí por encima de `{python} f"{min(correlaciones.loc['Channel2', 'Channel3'], correlaciones.loc['Channel2', 'Channel4'], correlaciones.loc['Channel3', 'Channel4']):.2f}"`, y son `{python} len(df)` filas para separarlos, más el retraso con el que vende cada anuncio.

::: aside
Esto es lo que te van a poner delante en una reunión de marketing.
:::
...
## Quito los medios y quedan tres parámetros: el nivel, los controles y el ruido
...
- Todo va estandarizado: `mu` es el nivel del KPI y `gamma_c`, cuánto lo mueve ese control cuando sube una desviación típica.
...
## Esto no se afirma: se simula. Y con las prioris de Meridian salen semanas en negativo
...
titulos = ["Normal(0, 1) y Exponential(1)", "Las prioris de Meridian"]
...
A la derecha, el `{python} f"{pct_negativas_meridian:.1f}"` % de las semanas simuladas factura menos que nada.
...
## Se ajusta en tres segundos, y antes de leer la posteriori miro si ha funcionado
...
- `r_hat` máximo `{python} f"{resumen_esqueleto['r_hat'].max():.2f}"`, `ess_bulk` mínimo `{python} int(resumen_esqueleto['ess_bulk'].min())`, divergencias `{python} divergencias_esqueleto`.
...
## Catorce parámetros en vez de tres: ya no son tres segundos, y hay que volver a mirar
...
recoger datos. La regresión del principio decidió que el anuncio de esta semana no vende
nada la semana que viene, que es un supuesto brutal, y no aparecía en ninguna tabla. La
priori al menos está escrita y se puede auditar.
...
8. **"¿Qué pinta tienen los datos?"** Backup: `series`. Las 156 semanas dibujadas, ingresos
   arriba e inversión abajo. Sale del recorrido de la charla: si nadie pregunta, no se enseña.
9. **"¿Vale para mi TFM, para mi campo?"** Vale para cualquier sitio donde haya pocos
...
bullet de la slide del OLS con dibujo en vez de con un número.
:::
