...
## En PyMC queda así: priori, datos y verosimilitud
...
n_pasos = 2000
subida = 50
zoom = 200  # los pasos que enseña la slide de backup: con los dos mil no se distingue ninguno
...
dibuja_recorrido(ax1)  # los dos mil pasos, sin detalle: importa el montón, no el paso
...
::: {.notes}
La regla primero, en cuatro frases, sobre el recorrido de la izquierda: estás en un valor de
$p$, propones otro cercano, miras cuántas veces más plausible es —priori por verosimilitud,
nada más— y si es mejor, te mueves; si es peor, te mueves con esa probabilidad. Dos mil veces.

Aquí están los dos mil pasos y no se distingue ninguno, así que el paso va de palabra: un
tercio de las veces, más o menos, la propuesta es peor y se rechaza, y la cadena se queda
donde estaba. La zona sombreada del principio es el arranque en 0,10, deliberadamente malo:
la cadena no sabe nada cuando empieza y esos primeros pasos son subida, no muestreo. Eso es
el `tune=2000` de la slide del modelo, que PyMC usa para colocarse y luego tira.

Si alguien quiere ver el paso y el rechazo, están en la slide de backup `caminata`: los
doscientos primeros pasos con marcador, y los tramos planos son los rechazos. Se saca ahí, en
el turno de preguntas, y no se enseña aquí.
...
Y ahora el truco, que es el título: como el paso se acepta según la plausibilidad relativa,
la cadena acaba pasando en cada zona un tiempo proporcional a lo plausible que es esa zona. El
recuento de visitas no se parece a la posteriori: es la posteriori. Y no se ha resuelto
ninguna integral por el camino.
...
que de verdad quieres decir en una reunión.
...
Si preguntan si el 30 % de aquí y el 0,24 del `R²` del OLS son lo mismo: no. `1 − σ²` es el
análogo bayesiano, no el `R²`; corre sobre el KPI ya estandarizado, no sobre los ingresos en
euros del bloque anterior. Ni la métrica ni la escala coinciden, aunque la comparación nos
favorezca. (Detalle sin importancia práctica si alguien lo pincha: es `1 − E[σ]²`, no
`1 − E[σ²]`; la diferencia es de 0,25 puntos.)
:::
...
7. **"¿Cómo funciona el muestreo por dentro?"** Backup: `caminata`. Los pasos y los
   rechazos, de cerca.
...
- `caminata` — "¿cómo funciona el muestreo por dentro?"
...
## Esa distribución no se ha calculado: se ha recorrido {#caminata}
...
::: {.notes}
Los doscientos primeros pasos de los dos mil, de cerca, que es la única forma de que se
distinga alguno. Señalar los tramos planos: son los rechazos, los pasos en los que la cadena
propone un valor peor y se queda donde estaba. Un tercio de las veces, más o menos.

La zona sombreada es la subida inicial desde el arranque en 0,10, que es deliberadamente
malo: la cadena no sabe nada cuando empieza. Eso es el `tune=2000` del modelo, que PyMC usa
para colocarse y luego tira.

Esto es un Metropolis escrito a mano. PyMC hace NUTS, que propone usando el gradiente y deja
un recorrido que parece ruido blanco: ahí no se ve ni un paso corto ni un rechazo, y por eso
esta slide no es la traza de PyMC. La idea de fondo es la misma.
:::
