...
::: {.column width="40%"}
- Competencia: +1 desviación típica.
- Yo: entre `{python} euros(hdi_competencia[0])` y `{python} euros(hdi_competencia[1])` € a la semana.
...
::: aside
Intervalo de credibilidad al 90 %, sombreado.
:::
...
y una semana cualquiera se aparta de lo previsto entre `{python} euros(hdi_sigma_euros[0])` y
`{python} euros(hdi_sigma_euros[1])` €. Es poco, y está bien que lo sea: el muestreo es
impecable y el modelo, flojo. Las dos cosas a la vez, que en medios vuelven.
:::

## El sentimiento probablemente suma, pero no sé cuánto
...
- Entre `{python} euros(hdi_sentimiento[0])` y `{python} euros(hdi_sentimiento[1])` € a la semana.
